...
ADDU  $R5,%ONE  //$R5 = $R5 +1
MOV   $R7,%SHIFT_INP //$R7 = SHIFT_INP
SLLV  $R7,%ONE 
MOV   $R6,$R0
JALR  $R1,%CHECK

// Test for LRT
.const %SHIFT_IP, 0x80000000
ADDU  $R5,%ONE   //Idk?????????????????????????????
MOV   $R7,%SHIFT_IP //$R7 = SHIFTIP
LRT   $R7,%ONE  //0x80000000 --> 0x00000001
SUBU  $R7,%ONE  //0x00000001 --> 0x00000000
MOV   $R6,$R0   //$R6 = $R0
JALR  $R1, %CHECK

//TEST for ROR
.const %ROR2,      0x80000000
.const %ROR1,      0x00000001
ADDU  $R5,%ONE
MOV   $R7,%ROR1
ROR   $R7,%ONE
MOV   $R6,%ROR2
